...
// FreeRTOS task to handle the game logic
void game_task(void *pvParameters) {
    ...
    while (1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(1000));  // Delay between rounds (FreeRTOS delay)
    }
}
#include <time.h>
...
int main() {
    ...
    xTaskCreate(game_task, "Game Task", 2048, NULL, 1, NULL);

    // Start the FreeRTOS scheduler
    vTaskStartScheduler();

    while (1) {
        tight_loop_contents();  // The scheduler is now running, this is just a placeholder
    }
